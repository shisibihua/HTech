define('calendar/cxcalendar', function(require, exports, module) {
/*!
 * jQuery cxCalendar
    ...
    (function (factory) {
        ...
        $.cxCalendar = function () {
            ...
            calendar.setOptions = function (opts) {
                ...
                // 缓存日期
                self.cacheDay = self.formatDate('YYYY-MM-DD', self.dom.el.val());
            ...
            calendar.build = function (isFirst) {
                ...
                if (isFirst === true) {
                    self.dom.pane = $('<div></div>', {
                        'class': 'cxcalendar'
                    });
                    self.dom.paneHd = $('<div></div>', {
                        'class': 'cxcalendar_hd'
                    }).appendTo(self.dom.pane).html('<a class="prev" href="javascript://" rel="prev"></a><a class="next" href="javascript://" rel="next"></a>');
                    self.dom.paneBd = $('<div></div>', {
                        'class': 'cxcalendar_bd'
                    }).appendTo(self.dom.pane);
                    self.dom.paneFt = $('<div></div>', {
                        'class': 'cxcalendar_ft'
                    }).appendTo(self.dom.pane);

                    self.dom.dateTxt = $('<div></div>', {
                        'class': 'intxt'
                    }).appendTo(self.dom.paneHd);
                    self.dom.dateSet = $('<div></div>', {
                        'class': 'inset'
                    }).appendTo(self.dom.paneHd);

                    self.dom.yearSet = $('<select></select>', {
                        'class': 'year'
                    }).appendTo(self.dom.dateSet);
                    self.dom.monthSet = $('<select></select>', {
                        'class': 'month'
                    }).appendTo(self.dom.dateSet);

                    self.dom.weekSet = $('<ul></ul>', {
                        'class': 'week'
                    }).appendTo(self.dom.paneBd);
                    self.dom.daySet = $('<ul></ul>', {
                        'class': 'days'
                    }).appendTo(self.dom.paneBd);

                    self.dom.dayTxt = $('<div></div>', {
                        'class': 'inday'
                    }).appendTo(self.dom.paneFt);
                    self.dom.timeSet = $('<div></div>', {
                        'class': 'intime'
                    }).appendTo(self.dom.paneFt);
                ...
                if (isFirst === true) {
                    // 面板及背景遮挡层插入到页面中
                    if (self.dom.pane.hasClass('cxcalendar_holyday')) {
                        self.dom.pane.appendTo('.wrap');
                        self.dom.pane.css({
                            'position': 'static',
                            'margin-top': '20px'
                        });
            ...
            calendar.gotoDate = function (year, month) {
                ...
                for (var i = 0; i < _dayMax; i++) {
                    ...
                    _title = this.formatDate('YYYY-MM-DD', _title);
