...
public class DeviceHostServiceImpl implements DeviceHostService {
    ...
    private static final String IFREECOMM_HD300B_TYPEINT="25";
    private static final String IFREECOMM_HD300B_FACTORY="HiteTech-HD300B";
    private static final String IFREECOMM_FACTORY="HiteTech-IFreeComm";
    private static final String HTECH_CMD="htech";
    private static final String ADD_IFREECOMM_TERMINAL="addTerminalByJsft";
    private static final String DEL_IFREECOMM_TERMINAL="delTerminalByJsft";
            ...
            if (reentrantLock.tryLock()) {
               ...
               int dspecId = DeviceTypeNameEnum.getDespecIdByType(deviceType.toLowerCase());
                ...
                try {
                    ...
                    //调用互动平台接口，保存设备到捷视飞通
                    if (result > 0 ) {
                        if(IFREECOMM_HD300B_TYPEINT.equals(typeInt)){
                            addIFreeCommTerminal(deviceHost);
                        }
                        resultMap.put("hostId", result);
                    } else {
                        //SETNAME_ERROR 这里表示添加设备异常
                        resultMap.put("hostId", SETNAME_ERROR);
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
    ...
    private void addIFreeCommTerminal(DeviceHost deviceHost){
        ...
        techParams.put("cmd_op",ADD_IFREECOMM_TERMINAL);
        ...
        JSONObject json=DeviceConnectUtil.sendGetRequestCommand(
        ...
        if(json!=null && Integer.parseInt(String.valueOf(json.get("code")))==0){
            //捷视飞通添加设备成功后，返回设备id
            int teminalCodeId=0;
            if(json.get("result")!=null && !"".equals(String.valueOf(json.get("result")))){
                try {
                    JSONObject jsonResult = JSONObject.fromObject(json.get("result"));
                    System.out.println(jsonResult.toString());
                    if (jsonResult != null) {
                        teminalCodeId = Integer.parseInt(String.valueOf(jsonResult.get("id")));
                    }
        ...
        for (int i = 0; i < hostIds.length; i++) {
            ...
            if(device!=null && !device.isEmpty()) {
                ...
                if (isDelete > 0) {
                    ...
                    if (IFREECOMM_HD300B_FACTORY.equals(device.get("host_factory"))) {
    ...
    private void deleteIFreeCommTerminal(Map<String,Object> device) {
        ...
        Map<String,String> techParams=new HashMap<>();
        techParams.put("cmd",HTECH_CMD);
        techParams.put("cmd_op",DEL_IFREECOMM_TERMINAL);
        techParams.put("url","http://"+mcuIp);
        techParams.put("id",String.valueOf(device.get("isTour")));
        if(device!=null && !device.isEmpty()){
            DeviceConnectUtil.sendGetRequestCommand(DeviceConnectUtil.getServiceConnect(DeviceConnectUtil.techServiceFlag),techParams);
        }
    }
...
//            String hostIp = device.getHostIpaddr();
    ...
    public String getHostIsTourByCode(String code) {
        if(code!=null && !"".equals(code)){
            return deviceHostMapper.getHostIsTourByCode(code.split(","));
        }
        return null;
    }

    @Override
    public DeviceHost getDeviceHostByMcuCode(String code) {
        if(code!=null && !"".equals(code)){
            return deviceHostMapper.getDeviceHostByMcuCode(code);
        }
        return null;
    }
}
